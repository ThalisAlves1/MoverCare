...
const SHIFT_OPTIONS = [
  { value: 'MANHA', label: 'Manhã', start: '07:00', end: '13:00' },
  { value: 'TARDE', label: 'Tarde', start: '13:00', end: '19:00' },
  { value: 'NOITE', label: 'Noite', start: '19:00', end: '07:00' },
  { value: 'DIURNO', label: 'Diurno', start: '07:00', end: '19:00' },
  { value: 'PLANTAO_12X36', label: 'Plantão 12x36', start: '07:00', end: '19:00' }
]
...
function getDateFromMonth(monthValue, day = 1) {
  const [year, month] = String(monthValue || getCurrentMonthValue()).split('-')
...
function normalizeScheduleRow(row = {}) {
  return {
    ...row,
    maqueiro_name: row.maqueiro?.full_name || row.maqueiro_name || row.full_name || 'Maqueiro',
    sector_name: row.sector?.name || row.sector_name || 'Setor não informado'
  }
}

function getMaqueiroDefaultSectorId(maqueiro) {
  return maqueiro?.sector_id || maqueiro?.current_sector_id || maqueiro?.currentSectorId || ''
}

function SchedulePage({ schedules, maqueiros, sectors, profile, refreshData, registerAudit, setGlobalSuccess, setGlobalError }) {
  const [selectedMonth, setSelectedMonth] = useState(getCurrentMonthValue())
  const [form, setForm] = useState(() => ({
    ...EMPTY_SCHEDULE_FORM,
    shift_date: getDateFromMonth(getCurrentMonthValue(), 1)
  }))
  const [quickForm, setQuickForm] = useState(() => ({
    maqueiro_id: '',
    sector_id: '',
    shift_name: 'PLANTAO_12X36',
    start_time: '07:00',
    end_time: '19:00',
    notes: 'Escala 12x36 automática'
  }))
  const [quickStartDate, setQuickStartDate] = useState(getDateFromMonth(getCurrentMonthValue(), 1))
  const [quickSelectedDates, setQuickSelectedDates] = useState([])
  const [selectedMaqueiroForDefault, setSelectedMaqueiroForDefault] = useState('')
  const [selectedDefaultSector, setSelectedDefaultSector] = useState('')
  const [copyFromMonth, setCopyFromMonth] = useState(getCurrentMonthValue())
  const [copyToMonth, setCopyToMonth] = useState(getCurrentMonthValue())
  const [swapScheduleA, setSwapScheduleA] = useState('')
  const [swapScheduleB, setSwapScheduleB] = useState('')
  const [saving, setSaving] = useState(false)
  const [localError, setLocalError] = useState('')
  const [localSuccess, setLocalSuccess] = useState('')

  const activeMaqueiros = useMemo(() => {
    return (maqueiros || [])
      .filter(item => item.active !== false)
      .slice()
      .sort((a, b) => getMaqueiroName(a).localeCompare(getMaqueiroName(b)))
  }, [maqueiros])
  ...
  const activeSectors = useMemo(() => {
    return (sectors || [])
      .filter(item => item.active !== false)
      .slice()
      .sort((a, b) => Number(a.order_position ?? 999) - Number(b.order_position ?? 999) || String(a.name).localeCompare(String(b.name)))
  }, [sectors])

  const monthSchedules = useMemo(() => {
    return (schedules || [])
      .map(normalizeScheduleRow)
      .filter(item => String(item.shift_date || '').slice(0, 7) === selectedMonth)
      .sort((a, b) => {
        ...
        return String(a.maqueiro_name || '').localeCompare(String(b.maqueiro_name || ''))
      })
  ...
  const todaySchedules = useMemo(() => {
    ...
    return (schedules || [])
      .map(normalizeScheduleRow)
      .filter(item => String(item.shift_date || '').slice(0, 10) === today && item.active !== false)
  }, [schedules])

  const daysInMonth = useMemo(() => {
    const [year, month] = String(selectedMonth || getCurrentMonthValue()).split('-').map(Number)
  ...
  const monthDays = useMemo(() => {
    return Array.from({ length: daysInMonth }).map((_, index) => {
      const day = index + 1
      const dateKey = getDateFromMonth(selectedMonth, day)
      const date = new Date(`${dateKey}T12:00:00`)
      return {
        day,
        dateKey,
        weekDay: date.toLocaleDateString('pt-BR', { weekday: 'short' }).replace('.', ''),
        schedules: monthSchedules.filter(item => String(item.shift_date || '').slice(0, 10) === dateKey)
      }
    })
  }, [daysInMonth, monthSchedules, selectedMonth])

  const coverageByDay = useMemo(() => {
    const map = new Map()
    monthSchedules.forEach(item => {
      const key = String(item.shift_date || '').slice(0, 10)
      map.set(key, (map.get(key) || 0) + 1)
    })
    return map
  }, [monthSchedules])

  const selectedSwapA = monthSchedules.find(item => item.id === swapScheduleA)
  const selectedSwapB = monthSchedules.find(item => item.id === swapScheduleB)
  ...
  function applyShiftPreset(value) {
    const preset = SHIFT_OPTIONS.find(item => item.value === value)
    setForm(prev => ({
      ...prev,
      shift_name: value,
      start_time: preset?.start || prev.start_time,
      end_time: preset?.end || prev.end_time
    }))
  }

  function applyQuickShiftPreset(value) {
    const preset = SHIFT_OPTIONS.find(item => item.value === value)
    setQuickForm(prev => ({
      ...prev,
      shift_name: value,
  ...
  function resetForm() {
    setForm({
      ...EMPTY_SCHEDULE_FORM,
      shift_date: getDateFromMonth(selectedMonth, 1)
    })
    setLocalError('')
    setLocalSuccess('')
  }
  ...
  function editSchedule(schedule) {
    setForm({
      id: schedule.id,
      maqueiro_id: schedule.maqueiro_id || schedule.maqueiro?.id || '',
      sector_id: schedule.sector_id || schedule.sector?.id || '',
      shift_date: String(schedule.shift_date || '').slice(0, 10),
      shift_name: schedule.shift_name || 'DIURNO',
      start_time: String(schedule.start_time || '07:00').slice(0, 5),
      end_time: String(schedule.end_time || '19:00').slice(0, 5),
  ...
  function toggleQuickDate(dateKey) {
    setQuickSelectedDates(prev => {
      if (prev.includes(dateKey)) return prev.filter(item => item !== dateKey)
      return [...prev, dateKey].sort()
    })
  }

  function clearQuickDates() {
    setQuickSelectedDates([])
  }

  function markAllMonthDays() {
    setQuickSelectedDates(monthDays.map(item => item.dateKey))
  }

  function mark12x36Days() {
    setLocalError('')
    setLocalSuccess('')

    if (!quickStartDate || String(quickStartDate).slice(0, 7) !== selectedMonth) {
      setLocalError('A data inicial do 12x36 precisa estar dentro do mês selecionado.')
      return
    }

    const startDay = Number(String(quickStartDate).slice(8, 10))
    const dates = []

    for (let day = startDay; day <= daysInMonth; day += 2) {
      dates.push(getDateFromMonth(selectedMonth, day))
    }

    setQuickSelectedDates(dates)
    applyQuickShiftPreset('PLANTAO_12X36')
    setLocalSuccess(`12x36 marcado automaticamente: ${dates.length} plantão${dates.length === 1 ? '' : 'ões'}.`)
  }

  function chooseScheduleForSwap(scheduleId) {
    if (swapScheduleA === scheduleId) {
      setSwapScheduleA('')
      return
    }

    if (swapScheduleB === scheduleId) {
      setSwapScheduleB('')
      return
    }

    if (!swapScheduleA) {
      setSwapScheduleA(scheduleId)
      return
    }

    if (!swapScheduleB) {
      setSwapScheduleB(scheduleId)
      return
    }
  ...
  function scheduleOptionLabel(schedule) {
    if (!schedule) return ''
    return `${formatDateOnly(schedule.shift_date)} • ${String(schedule.start_time || '').slice(0, 5)} • ${schedule.maqueiro_name} • ${schedule.sector_name}`
  }
  ...
  async function saveSchedule(event) {
    ...
    try {
      if (!form.maqueiro_id) throw new Error('Selecione o maqueiro.')
      if (!form.sector_id) throw new Error('Selecione o setor.')
      if (!form.shift_date) throw new Error('Informe a data da escala.')
      if (!form.shift_name) throw new Error('Informe o turno.')
      if (!form.start_time || !form.end_time) throw new Error('Informe o horário inicial e final.')

      const { error: rpcError } = await supabase.rpc('coordinator_upsert_maqueiro_schedule', {
        p_schedule_id: form.id,
        p_maqueiro_id: form.maqueiro_id,
        p_sector_id: form.sector_id,
        p_shift_date: form.shift_date,
        p_shift_name: form.shift_name,
      ...
      const maqueiro = activeMaqueiros.find(item => item.id === form.maqueiro_id)
      const sector = activeSectors.find(item => item.id === form.sector_id)
      await registerAudit?.('ESCALA', `${form.id ? 'Atualizou' : 'Criou'} escala de ${getMaqueiroName(maqueiro)} em ${sector?.name || 'setor'} para ${formatDateOnly(form.shift_date)}`, {
        scheduleId: form.id,
        maqueiroId: form.maqueiro_id,
        sectorId: form.sector_id,
        shiftDate: form.shift_date
      })

      await refreshData()
      setLocalSuccess('Escala salva com sucesso.')
      setGlobalSuccess?.('Escala salva com sucesso.')
      resetForm()
    } catch (error) {
      const message = error?.message || 'Erro ao salvar escala.'
  ...
  async function saveSelectedDates(event) {
    event.preventDefault()
    setSaving(true)
    setLocalError('')
    setLocalSuccess('')
    ...
    try {
      if (!quickForm.maqueiro_id) throw new Error('Selecione o maqueiro para montar a escala.')
      if (!quickForm.sector_id) throw new Error('Selecione o setor da escala.')
      if (!quickSelectedDates.length) throw new Error('Marque pelo menos uma data na tabela.')
      if (!quickForm.shift_name) throw new Error('Selecione o turno.')
      if (!quickForm.start_time || !quickForm.end_time) throw new Error('Informe início e fim do plantão.')

      for (const dateKey of quickSelectedDates) {
        const { error: rpcError } = await supabase.rpc('coordinator_upsert_maqueiro_schedule', {
          p_schedule_id: null,
          p_maqueiro_id: quickForm.maqueiro_id,
          p_sector_id: quickForm.sector_id,
          p_shift_date: dateKey,
          p_shift_name: quickForm.shift_name,
          p_start_time: quickForm.start_time,
          p_end_time: quickForm.end_time,
          p_notes: quickForm.notes || '',
          p_active: true
        })
      ...
      const maqueiro = activeMaqueiros.find(item => item.id === quickForm.maqueiro_id)
      const sector = activeSectors.find(item => item.id === quickForm.sector_id)
      await registerAudit?.('ESCALA 12X36', `Montou ${quickSelectedDates.length} plantões para ${getMaqueiroName(maqueiro)} em ${sector?.name || 'setor'} no mês ${getMonthLabel(selectedMonth)}`, {
        maqueiroId: quickForm.maqueiro_id,
        sectorId: quickForm.sector_id,
        month: selectedMonth,
        dates: quickSelectedDates
      })

      await refreshData()
      setLocalSuccess(`${quickSelectedDates.length} plantão${quickSelectedDates.length === 1 ? '' : 'ões'} salvo${quickSelectedDates.length === 1 ? '' : 's'} com sucesso.`)
      setGlobalSuccess?.('Escala mensal salva com sucesso.')
      setQuickSelectedDates([])
    } catch (error) {
      const message = error?.message || 'Erro ao salvar escala mensal.'
      setLocalError(message)
      setGlobalError?.(message)
    } finally {
      setSaving(false)
  ...
  async function deleteSchedule(schedule) {
    const ok = window.confirm(`Cancelar/remover escala de ${schedule.maqueiro_name || getMaqueiroName(schedule.maqueiro)} em ${formatDateOnly(schedule.shift_date)}?`)
    if (!ok) return
    ...
    try {
      const { error: rpcError } = await supabase.rpc('coordinator_delete_maqueiro_schedule', {
        p_schedule_id: schedule.id
      })

      if (rpcError) throw rpcError

      await registerAudit?.('ESCALA', `Removeu escala de ${schedule.maqueiro_name || getMaqueiroName(schedule.maqueiro)} em ${formatDateOnly(schedule.shift_date)}`, { scheduleId: schedule.id })
      await refreshData()
      setLocalSuccess('Escala removida.')
      setGlobalSuccess?.('Escala removida.')
    } catch (error) {
  ...
  async function swapSchedules(event) {
    event.preventDefault()
    setSaving(true)
    setLocalError('')
    setLocalSuccess('')
    ...
    try {
      if (!swapScheduleA || !swapScheduleB) throw new Error('Selecione duas escalas para realizar a troca.')
      if (swapScheduleA === swapScheduleB) throw new Error('Selecione duas escalas diferentes.')

      const { error: rpcError } = await supabase.rpc('coordinator_swap_maqueiro_schedules', {
        p_schedule_a_id: swapScheduleA,
        p_schedule_b_id: swapScheduleB,
        p_reason: 'Troca de plantão realizada pelo coordenador'
      })

      if (rpcError) throw rpcError

      await registerAudit?.('TROCA DE ESCALA', `Realizou troca entre ${scheduleOptionLabel(selectedSwapA)} e ${scheduleOptionLabel(selectedSwapB)}`, {
        scheduleA: swapScheduleA,
        scheduleB: swapScheduleB
      })

      await refreshData()
      setSwapScheduleA('')
      setSwapScheduleB('')
      setLocalSuccess('Troca realizada com sucesso.')
      setGlobalSuccess?.('Troca de escala realizada com sucesso.')
    } catch (error) {
      const message = error?.message || 'Erro ao realizar troca.'
      setLocalError(message)
      setGlobalError?.(message)
    } finally {
      setSaving(false)
  ...
  async function saveDefaultSector(event) {
    event.preventDefault()
    setSaving(true)
    setLocalError('')
    setLocalSuccess('')
    ...
    try {
      if (!selectedMaqueiroForDefault) throw new Error('Selecione o maqueiro.')
      if (!selectedDefaultSector) throw new Error('Selecione o setor padrão.')

      const { error: rpcError } = await supabase.rpc('coordinator_update_maqueiro_default_sector', {
        p_maqueiro_id: selectedMaqueiroForDefault,
        p_sector_id: selectedDefaultSector
      })

      if (rpcError) throw rpcError

      const maqueiro = activeMaqueiros.find(item => item.id === selectedMaqueiroForDefault)
      const sector = activeSectors.find(item => item.id === selectedDefaultSector)
      await registerAudit?.('SETOR DO MAQUEIRO', `Definiu ${sector?.name || 'setor'} como setor padrão de ${getMaqueiroName(maqueiro)}`, {
        maqueiroId: selectedMaqueiroForDefault,
        sectorId: selectedDefaultSector
      })

      await refreshData()
      setLocalSuccess('Setor padrão atualizado.')
      setGlobalSuccess?.('Setor padrão do maqueiro atualizado.')
    } catch (error) {
      const message = error?.message || 'Erro ao atualizar setor padrão.'
      setLocalError(message)
      setGlobalError?.(message)
    } finally {
      setSaving(false)
  ...
  async function copyScheduleMonth(event) {
    event.preventDefault()
    setSaving(true)
    setLocalError('')
    setLocalSuccess('')
    ...
    try {
      if (!copyFromMonth || !copyToMonth) throw new Error('Informe os meses de origem e destino.')

      const { data, error: rpcError } = await supabase.rpc('coordinator_copy_maqueiro_schedule_month', {
        p_from_month: `${copyFromMonth}-01`,
        p_to_month: `${copyToMonth}-01`
      })

      if (rpcError) throw rpcError

      await registerAudit?.('ESCALA', `Copiou escala de ${getMonthLabel(copyFromMonth)} para ${getMonthLabel(copyToMonth)}`, {
        fromMonth: copyFromMonth,
        toMonth: copyToMonth
      })

      await refreshData()
      setLocalSuccess(`Escala copiada. Registros criados/atualizados: ${data ?? 0}.`)
      setGlobalSuccess?.('Escala do mês copiada com sucesso.')
    } catch (error) {
      const message = error?.message || 'Erro ao copiar escala.'
      setLocalError(message)
      setGlobalError?.(message)
    } finally {
      setSaving(false)
  ...
  return (
    <>
        ...
        right={
          <div className="filter-row">
            <input type="month" value={selectedMonth} onChange={(event) => {
              ...
              setQuickStartDate(getDateFromMonth(month, 1))
              setForm(prev => ({ ...prev, shift_date: getDateFromMonth(month, 1) }))
              setQuickSelectedDates([])
            }} />
            <button type="button" className="light-button" onClick={refreshData} disabled={saving}>Atualizar</button>
          </div>
        }
      />

      {(localError || localSuccess) && (
        <div className="schedule-message-row no-print">
          {localError && <div className="error schedule-inline-message">{localError}</div>}
          {localSuccess && <div className="success schedule-inline-message">{localSuccess}</div>}
        </div>
      )}

      <section className="schedule-metrics-grid">
        <MetricCard label="Maqueiros ativos" value={activeMaqueiros.length} sub="podem entrar na escala" tone="teal" />
        <MetricCard label="Setores ativos" value={activeSectors.length} sub="locais disponíveis" tone="blue" />
        <MetricCard label="Escalas no mês" value={monthSchedules.length} sub={getMonthLabel(selectedMonth)} tone="amber" />
        <MetricCard label="Escalas hoje" value={todaySchedules.length} sub={formatDateOnly(getCurrentDateValue())} tone="teal" />
      </section>

      <section className="panel schedule-builder-panel">
        <div className="panel-title-row">
          <div>
            <h2>Montagem rápida da escala</h2>
            <span className="count-badge">caixa de seleção • 12x36 automático</span>
          </div>
          <div className="schedule-selected-count">
            {quickSelectedDates.length} data{quickSelectedDates.length === 1 ? '' : 's'} marcada{quickSelectedDates.length === 1 ? '' : 's'}
          </div>
        </div>

        <form className="schedule-builder-form" onSubmit={saveSelectedDates}>
          <label>
            Maqueiro
            <select value={quickForm.maqueiro_id} onChange={(event) => updateQuickForm('maqueiro_id', event.target.value)}>
              <option value="">Selecione</option>
              {activeMaqueiros.map(maqueiro => (
                <option key={maqueiro.id} value={maqueiro.id}>{getMaqueiroName(maqueiro)}</option>
              ))}
            </select>
          </label>
          ...
          <label>
            Setor
            <select value={quickForm.sector_id} onChange={(event) => updateQuickForm('sector_id', event.target.value)}>
              <option value="">Selecione</option>
              {activeSectors.map(sector => (
                <option key={sector.id} value={sector.id}>{sector.name}</option>
              ))}
          ...
          <label>
            Turno
            <select value={quickForm.shift_name} onChange={(event) => applyQuickShiftPreset(event.target.value)}>
          ...
          <label>
            Início
            <input type="time" value={quickForm.start_time} onChange={(event) => updateQuickForm('start_time', event.target.value)} />
          </label>

          <label>
            Fim
            <input type="time" value={quickForm.end_time} onChange={(event) => updateQuickForm('end_time', event.target.value)} />
          </label>

          <label>
            Data inicial 12x36
            <input type="date" value={quickStartDate} onChange={(event) => setQuickStartDate(event.target.value)} />
          </label>

          <label className="schedule-builder-notes">
            Observação
            <input value={quickForm.notes} onChange={(event) => updateQuickForm('notes', event.target.value)} placeholder="Ex.: escala 12x36, cobertura, plantão extra" />
          </label>

          <div className="schedule-builder-actions">
            <button type="button" onClick={mark12x36Days} disabled={saving}>Marcar 12x36</button>
            <button type="button" className="light-button" onClick={markAllMonthDays} disabled={saving}>Marcar mês inteiro</button>
            <button type="button" className="light-button" onClick={clearQuickDates} disabled={saving}>Limpar datas</button>
            <button type="submit" disabled={saving || quickSelectedDates.length === 0}>{saving ? 'Salvando...' : 'Salvar datas marcadas'}</button>
          </div>
        </form>

        <div className="schedule-month-selector-table">
          <div className="schedule-month-selector-head">
            <span>Marcar</span>
            <span>Data</span>
            <span>Dia</span>
            <span>Plantões já cadastrados</span>
          </div>

          {monthDays.map(day => {
            const checked = quickSelectedDates.includes(day.dateKey)
            return (
              <label className={`schedule-month-selector-row ${checked ? 'selected' : ''}`} key={day.dateKey}>
                <span>
                  <input type="checkbox" checked={checked} onChange={() => toggleQuickDate(day.dateKey)} />
                </span>
                <strong>{formatDateOnly(day.dateKey)}</strong>
                <span>{day.weekDay}</span>
                <div className="schedule-day-chips">
                  {day.schedules.length === 0 && <small>Sem plantão</small>}
                  {day.schedules.map(schedule => (
                    <button type="button" key={schedule.id} className="schedule-mini-chip" onClick={(event) => {
                      event.preventDefault()
                      editSchedule(schedule)
                    }}>
                      {schedule.maqueiro_name} • {String(schedule.start_time || '').slice(0, 5)}
                    </button>
                  ))}
                </div>
              </label>
      ...
      <section className="schedule-layout schedule-secondary-layout">
        <article className="panel schedule-form-panel">
          <div className="panel-title-row">
            <div>
              <h2>{form.id ? 'Editar escala individual' : 'Lançamento individual'}</h2>
              <span className="count-badge">ajuste fino</span>
            </div>
            {form.id && <button type="button" className="light-button" onClick={resetForm}>Nova escala</button>}
          </div>

          <form className="schedule-form-grid" onSubmit={saveSchedule}>
            <label>
              Maqueiro
              <select value={form.maqueiro_id} onChange={(event) => updateForm('maqueiro_id', event.target.value)}>
                <option value="">Selecione</option>
                {activeMaqueiros.map(maqueiro => (
                  <option key={maqueiro.id} value={maqueiro.id}>{getMaqueiroName(maqueiro)}</option>
                ))}
            ...
            <label>
              Setor da escala
              <select value={form.sector_id} onChange={(event) => updateForm('sector_id', event.target.value)}>
                <option value="">Selecione</option>
                {activeSectors.map(sector => (
                  <option key={sector.id} value={sector.id}>{sector.name}</option>
                ))}
            ...
            <label>
              Data
              <input type="date" value={form.shift_date} onChange={(event) => updateForm('shift_date', event.target.value)} />
            </label>

            <label>
              Turno
              <select value={form.shift_name} onChange={(event) => applyShiftPreset(event.target.value)}>
                {SHIFT_OPTIONS.map(option => (
                  <option key={option.value} value={option.value}>{option.label}</option>
                ))}
              </select>
            ...
            <label className="schedule-form-notes">
              Observação
              <input value={form.notes} onChange={(event) => updateForm('notes', event.target.value)} placeholder="Ex.: cobertura, troca, restrição operacional" />
            </label>

            <label className="check-row schedule-active-check">
              <input type="checkbox" checked={form.active} onChange={(event) => updateForm('active', event.target.checked)} />
              Escala ativa
            </label>

            <div className="schedule-form-actions">
              <button type="submit" disabled={saving}>{saving ? 'Salvando...' : form.id ? 'Salvar alteração' : 'Adicionar escala'}</button>
              <button type="button" className="light-button" onClick={resetForm} disabled={saving}>Limpar</button>
            </div>
          </form>
        </article>

        <aside className="panel schedule-default-panel">
          <div className="panel-title-row">
            <div>
              <h2>Setor padrão</h2>
              <span className="count-badge">usado pelo app</span>
            </div>
          </div>

          <form className="schedule-default-form" onSubmit={saveDefaultSector}>
            <label>
              Maqueiro
              <select value={selectedMaqueiroForDefault} onChange={(event) => {
                const maqueiroId = event.target.value
                const maqueiro = activeMaqueiros.find(item => item.id === maqueiroId)
                setSelectedMaqueiroForDefault(maqueiroId)
                setSelectedDefaultSector(getMaqueiroDefaultSectorId(maqueiro))
              }}>
                <option value="">Selecione</option>
                {activeMaqueiros.map(maqueiro => (
                  <option key={maqueiro.id} value={maqueiro.id}>{getMaqueiroName(maqueiro)}</option>
            ...
            <label>
              Setor definido pelo coordenador
              <select value={selectedDefaultSector} onChange={(event) => setSelectedDefaultSector(event.target.value)}>
                <option value="">Selecione</option>
                {activeSectors.map(sector => (
                  <option key={sector.id} value={sector.id}>{sector.name}</option>
                ))}
            ...
            <button type="submit" disabled={saving}>Salvar setor padrão</button>
          </form>

          <div className="schedule-default-list">
            <h3>Maqueiros e setores</h3>
            {activeMaqueiros.slice(0, 8).map(maqueiro => (
              <div key={maqueiro.id}>
                <span className="mini-avatar">{String(getMaqueiroName(maqueiro)).slice(0, 1).toUpperCase()}</span>
                <strong>{getMaqueiroName(maqueiro)}</strong>
                <small>{getSectorNameFromMaqueiro(maqueiro) || 'Sem setor definido'}</small>
              </div>
            ))}
          </div>
        </aside>
      ...
      <section className="panel schedule-swap-panel no-print">
        <div className="panel-title-row">
          <div>
            <h2>Troca de plantão</h2>
            <span className="count-badge">selecione duas escalas</span>
          </div>
        </div>

        <form className="schedule-swap-form" onSubmit={swapSchedules}>
          <label>
            Escala 1
            <select value={swapScheduleA} onChange={(event) => setSwapScheduleA(event.target.value)}>
              <option value="">Selecione</option>
              {monthSchedules.map(schedule => (
                <option key={schedule.id} value={schedule.id}>{scheduleOptionLabel(schedule)}</option>
              ))}
            </select>
          </label>
          ...
          <label>
            Escala 2
            <select value={swapScheduleB} onChange={(event) => setSwapScheduleB(event.target.value)}>
              <option value="">Selecione</option>
              {monthSchedules.map(schedule => (
                <option key={schedule.id} value={schedule.id}>{scheduleOptionLabel(schedule)}</option>
              ))}
          ...
          <button type="submit" disabled={saving || !swapScheduleA || !swapScheduleB}>Realizar troca</button>
        </form>

        <div className="schedule-swap-preview">
          <div>
            <small>Escala 1</small>
            <strong>{selectedSwapA ? scheduleOptionLabel(selectedSwapA) : 'Nada selecionado'}</strong>
          </div>
          <div>
            <small>Escala 2</small>
            <strong>{selectedSwapB ? scheduleOptionLabel(selectedSwapB) : 'Nada selecionado'}</strong>
          </div>
        </div>
      </section>

      <section className="panel schedule-copy-panel no-print">
        <div className="panel-title-row">
          <div>
            <h2>Copiar escala mensal</h2>
            <span className="count-badge">atalho para montar mês novo</span>
          </div>
        </div>

        <form className="schedule-copy-form" onSubmit={copyScheduleMonth}>
          <label>
            Copiar de
            <input type="month" value={copyFromMonth} onChange={(event) => setCopyFromMonth(event.target.value)} />
          </label>
          <label>
            Para
            <input type="month" value={copyToMonth} onChange={(event) => setCopyToMonth(event.target.value)} />
          </label>
          <button type="submit" disabled={saving}>Copiar mês</button>
        </form>
      </section>
      ...
      <section className="panel schedule-table-panel">
        <div className="panel-title-row">
          <div>
            <h2>Escala cadastrada</h2>
            <span className="count-badge">{monthSchedules.length} registro{monthSchedules.length === 1 ? '' : 's'}</span>
          </div>

          <div className="filter-row">
            <button type="button" className="light-button" onClick={() => setForm(prev => ({ ...prev, shift_date: getDateFromMonth(selectedMonth, 1) }))}>Usar mês</button>
          </div>
        </div>

        <div className="schedule-calendar-strip no-print">
          {monthDays.map(day => {
            const count = coverageByDay.get(day.dateKey) || 0
            return (
              <button
                type="button"
                key={day.dateKey}
                className={count > 0 ? 'has-scale' : ''}
                onClick={() => setForm(prev => ({ ...prev, shift_date: day.dateKey }))}
              >
        ...
        <div className="schedule-table schedule-table-with-checkbox">
          ...
          {monthSchedules.length === 0 && <div className="empty-row">Nenhuma escala cadastrada para este mês.</div>}

          {monthSchedules.map(schedule => {
            const checkedForSwap = swapScheduleA === schedule.id || swapScheduleB === schedule.id
            return (
              <div className={`schedule-row ${checkedForSwap ? 'selected-for-swap' : ''}`} key={schedule.id}>
                <span>
                  <input type="checkbox" checked={checkedForSwap} onChange={() => chooseScheduleForSwap(schedule.id)} />
                </span>
                <span>{formatDateOnly(schedule.shift_date)}</span>
                <strong>{schedule.maqueiro_name}</strong>
                <span>{schedule.sector_name}</span>
                <span>{formatValue(schedule.shift_name)}</span>
                <span>{String(schedule.start_time || '').slice(0, 5)} às {String(schedule.end_time || '').slice(0, 5)}</span>
                <span><StatusPill value={schedule.active === false ? 'CANCELADO' : 'DISPONIVEL'} /></span>
                <span className="schedule-actions">
                  <button type="button" className="small light-button" onClick={() => editSchedule(schedule)}>Editar</button>
                  <button type="button" className="small danger" onClick={() => deleteSchedule(schedule)} disabled={saving}>Remover</button>
                </span>
              </div>
            )
          })}
...
function App() {
  ...
  async function loadSchedules() {
    const { data, error: schedulesError } = await supabase
      .from('maqueiro_monthly_schedules')
      .select(`
        id,
        maqueiro_id,
        sector_id,
        ...
        created_at,
        updated_at,
        maqueiro:maqueiro_id(id, full_name, email),
        sector:sector_id(id, name)
      `)
    ...
    if (schedulesError) {
      console.warn('Tabela de escala ainda não está disponível. Rode o SQL 13_escala_maqueiros.sql:', schedulesError.message)
      setSchedules([])
      return
    }
    ...
    const schedulesChannel = supabase
      ...
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'maqueiro_monthly_schedules'
        },
        refresh
      )
      .subscribe()
